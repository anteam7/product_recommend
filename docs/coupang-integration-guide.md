...
0. 🛑 **쿠팡 "필수 구매옵션 입력 의무화" 정책(2026-02-02 시행) — 해결된 통과 조건 (2026-09-01 확정)**.
   이 정책 이후 `coupang-register-batch-v2.mjs`(ggsan)의 옛 하드코딩 방식("수량만 EXPOSED, 나머지 NONE",
   값 포맷 `"1 박스"`처럼 숫자·단위 사이 공백 포함)이 막혀 2026-07-02 이후 실제 등록 성공 사례가 없었음.
   `scripts/bio77-register.mjs` 개발 중 21건 연속 실패 후 다음 조합으로 **실제 APPROVED + SELLING까지 확인**됨:
   - 카테고리 메타 최상위 `isAllowSingleItem`이 `false`인 카테고리는 진짜 옵션(변형) 등록이 필요 — 단일 SKU 등록 자체가
     불가하므로 **등록 전에 반드시 SKIP**할 것 (`getCategoryMeta(code).isAllowSingleItem`).
   - `true`인 카테고리는: **"수량"(또는 총 수량) + 개당캡슐/정·개당중량·개당용량 그룹(groupNumber) 중 실제 상품형태에
     맞는 것 하나, 이 둘을 동시에 `exposed:"EXPOSED"`**로 보낸다 (그룹의 나머지 둘은 아예 배열에서 생략).
   - 🔑 **`attributeValueName`은 숫자와 단위 사이에 공백을 넣지 않는다** (`"40.5g"`, `"1개"` — 기존 관행이던
     `"40.5 g"`, `"1 개"`는 "유효하지 않은 구매 옵션 값 혹은 단위가 존재합니다" 오류로 거절됨). 실제 검증 코드는
     `scripts/bio77-register.mjs`의 `buildItemAttributes()` 참고.
   - 승인요청(`PUT .../approvals`)을 등록 직후 바로 호출하면 `"'임시저장' 상태의 상품만 승인 요청 가능합니다"`로
     실패할 수 있음(전파 지연) — 1~2초 뒤 재시도하면 통과. 승인은 예상보다 빨라 즉시 APPROVED까지 가는 경우도 있음.
